...
// Total Couriers
$totalCouriersQuery = "SELECT COUNT(*) AS total_couriers FROM couriers WHERE user_id = '$user_id'";
...
// Pending Deliveries
$pendingDeliveriesQuery = "SELECT COUNT(*) AS pending_deliveries FROM couriers WHERE status = 'Pending' AND user_id = '$user_id'";
...
$fulfilledDeliveriesQuery = "SELECT COUNT(*) AS fulfilled_deliveries FROM couriers WHERE status = 'Fulfilled' AND user_id = '$user_id'";
...
$cancelledDeliveriesQuery = "SELECT COUNT(*) AS cancelled_deliveries FROM couriers WHERE status = 'Cancelled' AND user_id = '$user_id'";
...
// Total Revenue
$totalRevenueQuery = "SELECT SUM(price_pkr) AS total_revenue FROM couriers WHERE status = 'Fulfilled' AND user_id = '$user_id'";
...
// Get data for charts
$monthQuery = "SELECT MONTH(created_at) AS month, COUNT(courier_id) AS total_couriers FROM couriers WHERE user_id = '$user_id' GROUP BY MONTH(created_at)";
$monthResult = mysqli_query($connect, $monthQuery);

// Prepare data for the total couriers doughnut chart
$months = [];
        ...
        <div class="col-xxl-12 mb-6 order-0">
            <div class="card">
                <div class="d-flex align-items-start row">
                    <div class="col-sm-7">
                        <div class="card-body">
                            <h5 class="card-title text-primary mb-3">Hi Agent <?php echo $_SESSION['full_name'] ?> 🎉
                            </h5>
                            <p class="mb-6">
                                We're glad to see you. Explore your dashboard <br /> to manage your couriers
                                effectively.
                    ...
                    <div class='card-body'>
                        <div class='card-title d-flex align-items-start justify-content-between mb-4'>
                            <div style='background-color: #d6f5fc; color: rgb(3,195,236); '
                                class='avatar d-flex justify-content-center align-items-center rounded mx-auto'>
                                <i class='fa fa-industry mx-auto'></i>
                            </div>
                        </div>
                        <p class='mb-1'>Total Revenue</p>
                        <h4 class='card-title mb-3'><?php echo number_format($totalRevenue); ?> Rs</h4>
                    </div>
                </div>
            </div>
        </div>
